...
from mock_api import load_data
...
def record_transfer_blocked(state: Dict[str, Any], reason: str = "") -> Dict[str, Any]:
    ...
    return _event(
        state=state,
        event_type="mock_transfer_blocked",
        status="mock_only",
...
def run_business_flow(state: Dict[str, Any], fixtures: List[str] | None = None) -> Dict[str, Any]:
    ...
    if action["type"] != "none":
        events.append(
            _event(
                state=state,
                event_type=f"mock_{action['type']}",
                status="planned",
                order_id=order_id,
        ...
        events.append(
            _event(
                state=state,
                event_type="mock_qc_sop_proposal",
                status="drafted",
                order_id=order_id,
                payload={"ticket_type": ticket_type, "branch": sop_state.get("sop_branch")},
        ...
        events.append(
            _event(
                state=state,
                event_type="mock_private_domain_task",
                status="drafted",
                order_id=order_id,
                payload={"ticket_type": ticket_type, "branch": sop_state.get("sop_branch")},
