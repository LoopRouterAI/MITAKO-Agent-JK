@echo off
chcp 65001 >nul
title MITAKO 联调模式 - 甲方 Java 对接模拟终端
cd /d "%~dp0..\.."

echo ===================================================
echo   1. 启动甲方模拟服务：IdP / IM / 业务接口
echo   2. 写入 bpo-east OIDC 联调配置
echo   3. 以真实联调模式启动 MITAKO
echo ===================================================
...
start "MITAKO-Lab" cmd /k "cd /d %~dp0..\.. && set APP_PORT=8000&& set HANDOFF_BACKEND=hybrid&& set MITAKO_SSO_DEMO=0&& set MITAKO_AUTH_REQUIRED=0&& set MITAKO_PROTECTED_API_AUTH_REQUIRED=0&& set MITAKO_DEV_AUTH_BYPASS=1&& set MITAKO_BUSINESS_DEMO_API_ENABLED=1&& set CHATWOOT_MOCK=0&& set CHATWOOT_BASE_URL=http://127.0.0.1:9102&& set CHATWOOT_API_TOKEN=lab-token&& set CHATWOOT_ACCOUNT_ID=1&& set CHATWOOT_INBOX_ID=1&& %PY% main.py"
timeout /t 5 >nul
...
echo 日常开发请使用:   一键启动-Windows.bat
echo.
if %RC% neq 0 (
  echo [FAIL] self_integration_test 未全量通过
  pause
  exit /b 1
)
echo [OK] 联调自测通过
