...
- [ ] T068 固化新资料目录盘点、`ticket_` 标准化和批次/工单/场景唯一映射，隔离重名、场景冲突与缺快照案例
- [ ] T069 为订单快照适配器补齐去重后的 SKU、应发数量、物流包裹、赠品/抽赏规则和官方图片引用，继续排除隐私、价格与人工标签
- [ ] T070 先行补充映射、异常隔离、标签防泄露和适配器失败测试
- [ ] T071 实现官方商品图按任务限量拉取、主机白名单、响应校验、压缩与本地缓存，禁止启动或盘点阶段全量下载
- [ ] T072 将官方参考图以独立证据角色接入模型请求、API 与 HTML；下载失败时保留文字基线并输出受控降级原因
- [ ] T073 完成单任务、批任务、文件夹审核、缓存复用、恶意 URL、坏图和部分失败回归
- [ ] T074 对新资料执行真实只读 dry-run，生成映射完整度、异常隔离、字段覆盖和图片可达性抽样报告
- [ ] T075 更新甲方人话版 HTML、内部开发文档、配置说明和升级日志，并完成发布包独立启动验收
