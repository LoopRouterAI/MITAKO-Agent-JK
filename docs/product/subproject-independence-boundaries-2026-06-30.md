...
当前仓库是模块化单体，不是多个强独立仓库。主 FastAPI 进程承载客服 Agent、人工客服工作台、运营后台和业务适配层；视觉审核工作台作为独立 POC 服务运行。

## 当前边界

| 子系统 | 独立性 | 当前入口 | 数据边界 | 说明 |
| --- | --- | --- | --- | --- |
| 客服 Agent 主链路 | 中 | `/`、`/api/v1/chat/stream` | `handoff.db`、业务 fixture/适配层 | 负责安抚、解释、SOP 分流、转人工 |
| 人工客服工作台 | 中 | `/desk`、`/api/v1/handoff/*` | `handoff.db` | 负责队列、接单、回复、工单协作 |
...
2. 客服 Agent 只生成建议、补件要求、工单摘要和转人工动作，不自动退款、拒赔、补发或资料终审。
