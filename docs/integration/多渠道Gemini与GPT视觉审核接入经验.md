# 多渠道 Gemini 与 GPT 视觉审核接入经验

更新时间：2026-07-04

## 已验证结论

- `.env` 已配置 BananaRouter、API Yi、OpenAI 兼容通道所需 Key；报告不得输出密钥。
- API Yi Gemini 原生格式可用于 Gemini `generateContent` 链路。
...
1. BananaRouter Gemini 先试一次。
2. BananaRouter 软错误重试后仍失败，或硬错误，切 API Yi Gemini。
3. API Yi 软错误按 `Retry-After` 或指数退避重试。
4. Gemini 主审不可用时，再尝试 GPT/Qwen/Doubao 候选模型。
5. GPT-5.5 可作为高价值争议样本复核，不默认作为低成本主审。
...
| 场景 | 首选方案 | 复核候选 | 人工边界 |
|---|---|---|---|
| 开箱视频审核 | Gemini 3.5 Flash 基于抽帧和补充材料输出连续性、离镜、疑似剪辑、发错货信号 | GPT-5.5、Qwen、Doubao 做争议样本复核 | 低置信度、疑似剪辑、缺 SKU/主数据、模型分歧转人工 |
