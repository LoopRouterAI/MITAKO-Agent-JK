...
7. 主动停止、SSE 断连和全链路超时均能取消并等待服务端任务，取消/超时不写助手成功回复。
...
6. 本轮不新增持久化 `chat_turn` ledger；进程异常退出后的跨重启轮次恢复保留为剩余风险，只有甲方明确要求跨进程续跑或逐轮审计且本轮取消语义稳定后再立项。
...
- 客服编排没有消费 `MITAKO-FOUR-SCENE@20260814.1` 四场景唯一契约；`review_service` 只作为附件上下文出现，聊天链路仍用通用意图、通用材料话术和旧 SOP 分支独立判断，导致审核结论与客服回复存在两套真源。
- 旧回归只断言 `should_transfer=True`，没有要求队列回执；`main.py` 又把该计划态直接改写为“已转接”，因此测试固化了虚假转接而非识别它。
- `_build_product_inventory_reply()` 使用 `key in user_text or key in str(value)` 选择商品，目录键天然存在于自身商品字典字符串中，导致首个商品候选恒真命中并向用户返回无关库存、规格和发货信息。
- 前端 `AbortController` 只会停止浏览器读取；`chat_stream()` 创建的 LangGraph `asyncio.Task` 不监听客户端断连，也没有在 `finally` 中取消并等待。断连或点击停止后，服务端仍可能继续调用模型/工具、生成默认成功回复并写入会话。
...
请求生命周期沿用现有原语形成单一取消链：前端停止按钮调用当前轮次 `AbortController.abort()`；SSE 断连、全链路超时或同 session 新请求进入时，后端取消对应 `asyncio.Task` 并 `await` 收口。取消和超时不是成功终态，不发送 `done: completed`，不持久化助手成功回复，也不触发完成态动作。
...
```json
{
  ...
  "intent_codes": ["privacy_deletion"],
  "scenario_codes": ["privacy_compliance"],
  "confidence": 0.96,
...
复合诉求保留全部 `intent_codes` 和 `scenario_codes`；`intent_code` 表示本轮优先动作，`scenario_code` 表示首要业务场景。例如“退款并转人工”可使用 `intent_code=human_handoff`、`scenario_code=refund_progress`，但不得丢失退款进度原子诉求。
...
- 生成期间发送按钮切换为停止按钮；停止动作只终止当前轮次，恢复可再次发送状态，不把已收到的局部文本写入正式历史。
...
7. 客户端断连或主动停止：立即取消并等待服务端任务，不写助手成功回复。
8. 全链路超时：返回明确的超时失败状态；同一 session 同时只允许一个活动聊天任务，新请求不得与旧任务并发写同一会话。
...
6. 生命周期测试：前端停止、SSE 断连、全链路超时、同 session 单飞；取消/超时后没有 `done: completed` 和助手成功回复持久化。
7. 三轮稳定性测试：每个场景运行 3 次，比较结构化字段而非字面措辞。
8. Deeptokenai.cn 部署复验：记录版本握手、接口响应、截图和人工结论。
...
- API/Web/坐席状态差异 0 次。
- 工具失败均有明确降级入口。
- 主动停止、SSE 断连和超时均能取消服务端任务，残留活动任务 0，取消/超时成功回复写入 0。
...
4. 实现 P1 业务规则与回复保护。
5. 实现 P2 场景策略和失败降级。
6. 闭环前端停止、SSE 断连、服务端取消、全链路超时和同 session 单飞。
7. 接入 API/Web/坐席统一投影。
8. 完成三轮回归和人工复验。
9. 双仓提交、部署 Deeptokenai.cn、发布新版本。
...
- [ ] 客服编排只消费四场景公开结果，不复制 Prompt、Schema 或确定性 SOP，并保留复合诉求的主意图与业务场景。
- [ ] 旧 `should_transfer=True` 测试已改为验证真实队列回执，商品咨询无唯一 SKU/链接/订单行时只追问。
- [ ] 前端停止、SSE 断连、全链路超时和同 session 新请求都能取消并等待原服务端任务。
- [ ] 取消/超时不发送成功 `done`，不写入助手成功回复，不产生无回执完成态。
- [ ] PDF 15 案每案连续 3 轮通过，动作完成态均可回链真实 Mock 回执，API/Web/Pad/手机/坐席状态一致。
- [ ] 未接真实甲方系统，未恢复旧 Companion/陪伴/角色扮演/文字冒险能力，公开页面未暴露模型、Key、内部 Prompt 或调试参数。
