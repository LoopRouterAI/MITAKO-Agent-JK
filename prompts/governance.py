...
- 没有可信原速基准时，不得仅凭 PTS、帧率或主观运动速度断言视频已加速；只能标记为待确认的橙色风险。只有该不确定性实际妨碍默认一帧每秒下的关键事实判断，或与多项异常交叉印证时，才要求补原速或原始材料。
...
- 应发清单只能来自系统订单商品行、SKU、数量和版本化活动规则；不得从商品标题、系列名或宣传组合效果推导额外应发项。
- 合规视频必须能回链未拆封起点、视频内面单与订单物流一致，以及一镜到底覆盖箱内全部商品；单独面单照片不能自动绑定视频中的箱体。
- 无合规视频时只核验到手实物全家福、绿色自封袋和清晰面单三类静态图片；三类齐全只能转人工读取仓库实发明细，不能由 Agent 自行确认漏发或直接拒绝。
- 纸类叠放、背面或夹层检查只能作为确认漏发后的用户自查提醒，不得成为 Agent 判定漏发的前置条件。
