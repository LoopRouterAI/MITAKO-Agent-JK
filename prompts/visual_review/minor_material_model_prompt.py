...
8. 仅在 identity_age 检查中评估独立支付合理性：以年龄判断基准日期计算，先输出 age_band。清晰资料显示低于策略阈值 {policy.get('independent_payment_min_age', 10)} 岁时输出 under_10，10 至 17 岁输出 10_to_17，已满 18 岁输出 18_or_over，看不清或主体不明输出 unknown；禁止把监护人的出生日期当作未成年人年龄。只有 age_band=under_10 时 low_age 才能为 true，age_band=10_to_17 或 18_or_over 时必须为 false，unknown 时必须为 null。仅当未成年人主体映射清楚、出生信息清晰、基准日期存在且跨材料无冲突时，才能判断 under_nine；未满 9 周岁输出 true，已满 9 周岁输出 false，证据不足输出 null。只有上述条件全部满足且年龄证据清晰时 age_confidence 才能为 high；部分可读或存在任一不确定项时为 low，无法判断时为 unknown。其他检查固定输出 under_nine=null、age_confidence=unknown。低龄且 payment_password_access、guardian_discovery_process 任一项不是 matched 时，payment_capability_risk 才输出 high；两项均 matched 时输出 none。未满 9 周岁高置信只用于提示授权人员重点核对独立支付能力，不是法律结论，不得仅凭年龄给支持或拒绝结论，也不得输出具体年龄或出生日期。
