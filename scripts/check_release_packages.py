...
V31_RELEASE_RECEIPT = "docs/release/2026-08-19-v3.1-beta-release-receipt.md"
...
def _verify_customer(zip_path: Path, root: Path, expected_commit: str) -> dict[str, Any]:
    ...
    _assert(any(name.endswith("customer_service/contracts.pyc") for name in runtime_names), "甲方运行时缺少客服状态契约")
    _assert(any(name.endswith("customer_service/intent_router.pyc") for name in runtime_names), "甲方运行时缺少确定性意图路由")
    _assert(any(name.endswith("customer_service/action_state.pyc") for name in runtime_names), "甲方运行时缺少工具回执状态模块")
    _assert(any(name.endswith("customer_service/scenario_policy.pyc") for name in runtime_names), "甲方运行时缺少客服场景策略")
    _assert(any(name.endswith("customer_service/reply_guard.pyc") for name in runtime_names), "甲方运行时缺少客服回复守卫")
    _assert(any(name.endswith("customer_service/public_projection.pyc") for name in runtime_names), "甲方运行时缺少客服公开状态投影")
